		...
		try {
			...
			if (null != job) {
				...
				if (null == trigger) {// 不存在任务
					...
					// 把任务插入数据库
					/*
					 * int result = quartzBS.add(job); if(result!=0){
					 * model.addAttribute("msg", "您的任务创建成功！"); }else{
					...
					// 更新数据库中的任务
					/*
					 * int result = quartzBS.update(job); if(result==1){
					 * model.addAttribute("msg", "您的任务更新成功！"); }else{
